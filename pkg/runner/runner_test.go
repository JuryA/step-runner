  ...
  - name: greet_previous
    ...
    inputs:
      name: ${{steps.greet_sponge_bob.outputs.name}}`,
		wantErr: errors.New(`failed step "greet_previous": Cannot assign input "name" due to error: steps.greet_sponge_bob.outputs.name: the "greet_sponge_bob" was not found`),
	}, {
		name: "complex steps",
		yaml: `
spec: {}
...
steps:
  - name: greet_steppy
    ...
    inputs:
      name: ${{ env.NAME }}
`,
		wantErr: errors.New(`failed step "greet_steppy": Cannot assign input "name" due to error: env.NAME: the "NAME" was not found`),
	}, {
...
`,
		wantErr: fmt.Errorf(`failed step "greeting": Cannot assign input "name" due to error: non-sensitive input cannot derive ` +
