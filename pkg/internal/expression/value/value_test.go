...
func TestObjectAll(t *testing.T) {
	...
	iter, err := obj.ObjectAll()
	require.NoError(t, err)

	expected := []string{"hello", "world", "foo", "bar"}
	idx := 0
	for key, val := range iter {
		assert.Equal(t, expected[idx], key.String())
		assert.Equal(t, expected[idx+1], val.String())
		idx += 2
	}

	num := Number(10)
	iter, err = num.ObjectAll()
...
func TestArrayAll(t *testing.T) {
	...
	for key, val := range iter {
		...
		assert.Equal(t, expected[idx], val.String())
		idx++
	}
}
...
func TestFloat(t *testing.T) {
	...
	f, err := num.Float()
	require.NoError(t, err)
	assert.Equal(t, new(big.Float).SetPrec(defaultPrecision).SetFloat64(1.99), f)
