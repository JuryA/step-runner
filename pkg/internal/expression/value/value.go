...
func (v Value) Get(key Value) (Value, error) {
	...
	default:
		return null, fmt.Errorf("%w: attribute access works for object or array, not %v", ErrInvalidKind, v.kind)
	}
}

// ObjectAll returns the keys and values of an object.
func (v Value) ObjectAll() (iter.Seq2[Value, Value], error) {
	if v.kind != ObjectKind {
		return nil, fmt.Errorf("%w: requires object, not %v", ErrInvalidKind, v.kind)
	}

	return v.v.(Mapper).All(), nil
}

// ArrayAll returns the index and values of an array.
func (v Value) ArrayAll() (iter.Seq2[int, Value], error) {
	if v.kind != ArrayKind {
		return nil, fmt.Errorf("%w: requires array, not %v", ErrInvalidKind, v.kind)
	}

	return v.v.(Indexer).All(), nil
}

// Float returns a number as a big.Float
func (v Value) Float() (*big.Float, error) {
	if v.kind != NumberKind {
		return nil, fmt.Errorf("%w: conversion to float requires number, not %v", ErrInvalidKind, v.kind)
	}

	return new(big.Float).Copy(v.v.(*big.Float)), nil
}
...
// getBigFloat returns a new float with the default precision set.
